        ...
        level: [
          {
            ...
            enemies:[
                new enemy(2350,  420,0, "img/cactus.png"),
                new enemy(2750,  420,1, "img/cactus.png")
                ]
...
function animate(){
	...
	passo.play()
    levels.level[0].backgrounds.forEach(Layer =>{
        Layer.update()
        Layer.draw()
    })
    levels.level[0].pavements.forEach(pavement =>{
        pavement.draw()
    })
	levels.level[0].stones.forEach(stone =>{
        stone.draw()
    })
	player.update()
	player.draw()
    levels.level[0].platforms.forEach(platform =>{
        platform.update(player)
        platform.draw()
    })
   
    levels.level[0].enemies.forEach(enemy =>{
        enemy.update(player)
        enemy.draw()
    })
    ...
    if(right_pressed && player.position.x < 400){
        ...
        levels.level[0].enemies.forEach((enemy) =>{
			...
			if(levels.level[0].pavements[0].position.x == 0){
				inc = 0
				stop = 0
			}
			else{
    ...
    if(player.position.y + player.height <= pavement.position.y 
        ...
        if(player.position.y + player.height >= levels.level[0].stones[2].position.y
            && player.position.y + player.height + player.velocity.y >= levels.level[0].stones[2].position.y 
            && player.position.x + player.width >= levels.level[0].stones[2].position.x + 30 
            && player.position.x + player.width <= levels.level[0].stones[2].position.x + levels.level[0].stones[2].image.width + 30
            )
            {
